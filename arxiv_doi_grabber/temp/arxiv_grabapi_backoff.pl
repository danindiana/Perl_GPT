...
# Function to extract and store metadata using the arXiv API with exponential backoff
sub extract_arxiv_metadata {
    ...
    my $retry_count = 0;
    my $max_retries = 5;
    my $backoff_time = 1;  # Start with 1 second backoff
    ...
    while ($retry_count < $max_retries) {
        ...
        if ($response->is_success) {
            ...
            $retry_count++;
            print "Retrying... (Attempt $retry_count of $max_retries) after $backoff_time seconds\n";
            sleep($backoff_time);
            $backoff_time *= 2;  # Exponential backoff (1, 2, 4, 8, 16 seconds)
        }
    }
